...
import { getCompanySpeech } from "@/data/speechTexts";
...
export default function CompanyDiagnosis() {
  ...
  const {
    ...
    isLastQuestion,
    allAnswered,
  } = useDiagnosis("company", COMPANY_QUESTIONS);

  const [showEmailInput, setShowEmailInput] = useState(false);
  const [showLoading, setShowLoading] = useState(false);
  const [isSaving, setIsSaving] = useState(false);
  const question = COMPANY_QUESTIONS[currentQuestion];
  const speech = getCompanySpeech(currentQuestion);
    ...
    async (email: string) => {
      ...
      try {
        ...
        const encoded = encodeURIComponent(
          JSON.stringify({ type: "company", answers })
        );
        setShowEmailInput(false);
        setShowLoading(true);
        setTimeout(() => {
